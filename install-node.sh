...
if [ $CORDA_NODE = "Notary" ]; then export CORDA_NOTARY_IP=localhost; elif [ $CORDA_NODE = "London" ]; then export CORDA_LONDON_IP=localhost; elif [ $CORDA_NODE = "NewYork" ]; then export CORDA_NEW_YORK_IP=localhost; elif [ $CORDA_NODE = "Paris" ]; then export CORDA_PARIS_IP=localhost; fi
cordaHome=/opt/corda;
mkdir $cordaHome;
...
if [ $CORDA_HOME != "Notary"]; then echo "
    ...
    " >> /etc/init/corda-webserver.conf; fi